...
class AWB_Frontend {
    ...
    public static function init(){
        ...
        // Do not render the builder in the single product summary. Instead we
        // output it just before the add to cart button so that the size
        // controls appear directly beneath the standard variation selectors.
        // add_action( 'woocommerce_single_product_summary', array( __CLASS__, 'render' ), 31 );
        add_action( 'woocommerce_before_add_to_cart_button', array( __CLASS__, 'render' ), 12 );
    ...
    public static function render(){
        ...
        // The old price and preview button container has been removed. The
        // pricing and call‑to‑action now live outside of the AWB box (see
        // below). We still calculate prices via JavaScript, but we no
        // longer output them here.
        // $show_preview_btn is kept for backwards compatibility but not
        // used directly in this section.
        $show_preview_btn = (bool) get_post_meta( $pid, '_awb_show_preview_btn', true );
        // Close the AWB box before outputting the order bar and hidden fields
        echo '</div>';
        // --- Custom order bar containing preview button, price info and delivery time ---
        // Always show the preview button (builder call‑to‑action). The button
        // triggers the modal via JS and reads 'Anpassen & Bestellen'.
        echo '<div class="awb-order-bar">';
        echo '  <button type="button" class="button awb-preview">'. esc_html__( 'Anpassen & Bestellen', 'ai-wallpaper-builder' ) .'</button>';
        echo '  <div class="awb-order-info">';
        echo '    <div class="total-price"></div>';
        echo '    <div class="sqm-price"></div>';
        echo '    <div class="awb-inc-vat">'. esc_html__( 'inkl. MwSt., zzgl. Versand', 'ai-wallpaper-builder' ) .'</div>';
        echo '  </div>';
        // Delivery time text; use product meta if set, otherwise fall back to default
        $delivery_text = get_post_meta( $pid, '_awb_delivery_text', true );
        if ( ! $delivery_text ) { $delivery_text = '3 bis 5 Tage*'; }
        echo '  <div class="awb-delivery-time">'. esc_html__( 'Lieferzeit:', 'ai-wallpaper-builder' ) . ' ' . esc_html( $delivery_text ) .'</div>';
        echo '</div>';
        // Hidden meta fields are appended outside the AWB box so JS can move
        // them into the WooCommerce add‑to‑cart form. Each has the
        // aiw-hidden-meta class and will be relocated by frontend.js.
